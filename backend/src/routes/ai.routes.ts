...
aiRoutes.get('/dashboard-metrics', async (c) => {
  ...
  return c.json({
    ...
    optimizationAlerts: ((latest.recommendations as any[]) || []).length,
    trendData,
    categoryData,
    radarData,
    strengths: ((latest.insights as any[]) || []).map(i => i.title),
    weaknesses: ((latest.recommendations as any[]) || []).filter(r => r.impact === 'high').map(r => r.title)
  });
});
